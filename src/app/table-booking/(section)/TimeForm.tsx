...
const TimeForm: FC<TimeFormProps> = ({ mainform, setpage }) => {
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center gap-3">
      <p className="text-3xl font-semibold text-foreground">Request a reservation</p>
      <p className="text-foreground">Select your details and we`ll try get the best seats for you</p>
      <Form {...form}>
        <form
        ...
        >
          <div className="grid w-full grid-cols-1 gap-6 py-12 md:grid-cols-2 lg:grid-cols-3">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-foreground">No of Guests</FormLabel>
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    ...
                    <SelectContent className="max-h-[300px] bg-background text-foreground">
                      {guestNumbers.map((num) => (
                        <SelectItem key={num} value={num.toString()} className="bg-background text-foreground">
                  ...
                  >
                    <PopoverTrigger asChild>
                      <FormControl>
                        <Button
                          variant={"outline"}
                          className={`h-12 w-full justify-start border-muted bg-transparent text-left font-normal text-foreground hover:text-foreground`}
                        >
          ...
          {form.watch('date') &&
            <div className="flex w-full flex-col gap-4 py-12">
              <p className="text-foreground">Choose an available time slot:</p>
              <div className="custom-scrollbar grid h-[250px] grid-cols-2 gap-6 overflow-y-scroll md:grid-cols-4 lg:grid-cols-5">
                {generateTimeSlots().map((time) => (
                  <div
                    ...
                    className={cn(
                      "flex h-full w-full cursor-pointer items-center justify-center border-[1px] border-gray-800 py-4 text-foreground",
          ...
          <Button
            className="w-fit px-6 py-6 bg-black text-foreground hover:bg-black/50"
            disabled={disable}
          >
            Book A Table
          </Button>
